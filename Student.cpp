...
Student :: Student(int id, string n, int a, float gpa){
    this->studentID=id;
    this->name=n;
    this->AcademicYear=a;
    this->gpa=gpa;
...
void Student :: displayRecordstoN(const string& filePath, int n) {
...
void Student :: readPrimaryIndex(const string& filename, map<string, int>& primaryIndex) {
...
void Student :: readSecondaryIndex(const string& filename, multimap<string, int>& secondaryIndex) {
    ifstream file(filename);
    if (!file.is_open()) {
...
void Student :: displayRecord(const string& filename, int offset) {
    ifstream file(filename);
    if (!file.is_open()) {
...
int Student :: searchByName(const string& n,const multimap<string, int>& secondaryIndex, const string& dataFilename,
...
int Student :: searchByID(int ID, const map<string, int>& primaryIndex) {
...
void Student :: updateHeader(const string& filename, int header) {
...
string Student::readHeader(const string& filename) {
    ifstream file(filename);
    if (!file.is_open()) {
        cerr << "Error: Unable to open file." << endl;
        return "";
...
void Student::markRecordAsDeleted(const string& filename, int offset, const string& header, int recordSize) {
    fstream file(filename, ios::in | ios::out);
    if (!file.is_open()) {
        cerr << "Error: Unable to open file." << endl;
        return;
...
void Student::updatePrimaryIndex(const string& filename, map<string, int>& primaryIndex) {
...
void Student::updateSecondaryIndex(const string& filename, multimap<string, int>& secondaryIndex) {
    ofstream file(filename, ios::trunc);
    if (!file.is_open()) {
...
string Student::searchNameByID(int id, const multimap<string, int>& secondaryIndex) {
    for (const auto& entry : secondaryIndex) {
        if (entry.second == id) {
            return entry.first;
        }
...
string Student::searchIDByoffset(int offset, map<string, int>& primaryIndex) {
    for (const auto& entry : primaryIndex) {
...
void Student::DeleteRecord(string n ,const string& primaryIndexFile,const string& secondaryIndexFile,
...
void Student::DeleteRecord(int id,const string& primaryIndexFile,const string& secondaryIndexFile,
                           const string& dataFile, map<string, int>& primaryIndex, multimap<string, int>& secondaryIndex) {
    string header = readHeader(dataFile);
    int offset = -1;
    string name;
...
void Student::InsertRecord(const Student& student, const string& primaryIndexFile, const string& secondaryIndexFile,
                        const string& dataFile, map<string, int>& primaryIndex, multimap<string, int>& secondaryIndex) {
    ...
    gpaStream << fixed << setprecision(1) << student.gpa;
    string formattedGpa = gpaStream.str();

    string newStudent = to_string(student.studentID) + "|" + student.name + "|" +
                        to_string(student.AcademicYear) + "|" + formattedGpa + "|";
    ...
    if (header != -1 && inserted) {
        ...
        primaryIndex[to_string(student.studentID)] = offset-newSize;
    }else{
        file.seekp(0,ios::end);
        offset=file.tellp();
        primaryIndex[to_string(student.studentID)] = offset-newSize;
    ...
    istringstream iss(student.name);
    string n;
    iss >> n;  // Extract the first word
    secondaryIndex.insert({n, student.studentID});
...
void Student::UpdateRecord(string n, const string& primaryIndexFile, const string& secondaryIndexFile,
                           const string& dataFile, map<string, int>& primaryIndex, multimap<string, int>& secondaryIndex) {
    fstream file(dataFile, ios::in | ios::out );
    ...
    if (size == newSize) {
        ...
        Student stu;
        stu.studentID=ID,stu.name=na,stu.AcademicYear=AcYear,stu.gpa=GPA;
...
void Student::UpdateRecord(int ID, const string& primaryIndexFile, const string& secondaryIndexFile,
                           const string& dataFile, map<string, int>& primaryIndex, multimap<string, int>& secondaryIndex) {
    fstream file(dataFile, ios::in | ios::out );
